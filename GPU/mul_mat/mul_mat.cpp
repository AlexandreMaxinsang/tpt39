...
//    }
     ...
     if (program == NULL)
	{
	 ...
	 if(err!=CL_SUCCESS) print_clbuild_errors(program,device);
   ...
   if (!d_A || !d_B || !d_C)
   {
    ...
    status = clEnqueueWriteBuffer(queue, d_A, CL_FALSE,
        0,mem_size_A, h_A, 0, NULL, &write_event[0]);
    checkError(status, "Failed to transfer input A");
	status = clEnqueueWriteBuffer(queue, d_B, CL_FALSE,
        0,mem_size_A, h_B, 0, NULL, &write_event[1]);
    checkError(status, "Failed to transfer input A");
    ...
    status = clEnqueueReadBuffer(queue, d_C, CL_TRUE,
        0, mem_size_C, h_C,0, NULL, NULL);
